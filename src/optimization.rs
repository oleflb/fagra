...
impl StateVisitor for Layout {
    type Error = SolverError;
    fn pool<T: Variable>(&mut self, pool: &mut StatePool<T>) -> Result<(), SolverError> {
        pool.prepare_trial();
...
impl<S> FactorVisitor<S> for Layout {
    type Error = SolverError;
    fn standalone<T: Factor<S>>(&mut self, pool: &mut FactorPool<T>) -> Result<(), SolverError> {
...
impl<S> FactorVisitor<S> for Cost<'_, S> {
    type Error = SolverError;
    fn standalone<T: Factor<S>>(&mut self, pool: &mut FactorPool<T>) -> Result<(), SolverError> {
...
fn cost<S, F: FactorSchema<S>>(states: &S, factors: &mut F) -> Result<f64, SolverError> {
    let mut visitor = Cost { states, total: 0.0 };
...
impl StateVisitor for Stage<'_> {
    type Error = EvaluationError;
    fn pool<T: Variable>(&mut self, pool: &mut StatePool<T>) -> Result<(), EvaluationError> {
        let count = pool
            .iter()
            .len()
            .checked_mul(T::DOF)
...
struct Accept;
struct Reject;
impl StateVisitor for Accept {
    type Error = Infallible;
    fn pool<T: Variable>(&mut self, pool: &mut StatePool<T>) -> Result<(), Infallible> {
        pool.accept_trial();
        Ok(())
    }
}
impl StateVisitor for Reject {
    type Error = Infallible;
    fn pool<T: Variable>(&mut self, pool: &mut StatePool<T>) -> Result<(), Infallible> {
        pool.reject_trial();
        Ok(())
...
impl<B: LeastSquaresBackend> LinearizationSink for CheckedSink<'_, B> {
    fn factor(
        &mut self,
        id: FactorId,
...
impl<S, B: LeastSquaresBackend> FactorVisitor<S> for Linearize<'_, '_, S, B> {
    type Error = SolverError;
    fn standalone<T: Factor<S>>(&mut self, pool: &mut FactorPool<T>) -> Result<(), SolverError> {
        for (id, factor) in pool.iter() {
...
pub(crate) fn norm_inf(values: &[f64]) -> Result<f64, SolverError> {
