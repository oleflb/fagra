...
//! [`factors!`], then insert values into a [`Problem`], [`optimize`](GaussNewton::solve_batch), and read
...
//! and bulk square-root [`marginalization`](Problem::marginalize) are implemented.
//! Checked [`replacement`](Problem::set), selected [`joint covariance`](Problem::joint_covariance),
//! and user-defined robust local models are supported. Covariance can reuse the
//! final optimizer model through [`GaussNewton::solve_batch_with_covariance`].
//! [`TrackedProblem`] provides opt-in mutation tracking; no incremental solver is implemented yet.
//!
...
//! read estimates through [`StateStore`]; the problem manages storage and handles.
//!
...
//! `Problem<States<f32>, Factors<f32>>` or `Problem<States<f64>, Factors<f64>>`.
...
//!
//! # Optimization
//! [`GaussNewton::solve_batch`] accepts a problem and [`OptimizeOptions`], returning
//! an [`OptimizeReport`]. The default backend is [`DenseNormalCholesky`]. Full steps may increase
...
pub use solver::{Problem, Solver};
pub use states::StateStore;
pub use tracked::{BatchProblem, ProblemChange, TrackedProblem};
